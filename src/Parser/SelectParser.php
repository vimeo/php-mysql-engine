			...
			switch ($token['type']) {
				...
				case TokenType::PAREN:
					...
					if ($query->needsSeparator) {
						...
						if (C\contains_key(keyset[TokenType::IDENTIFIER, TokenType::STRING_CONSTANT], $token['type']) && !$query->mostRecentHasAlias) {
				...
				case TokenType::RESERVED:
					switch ($token['value']) {
						case 'AS':
							...
							$this->pointer++;
							$next = $this->tokens[$this->pointer] ?? null;
							if ($next === null || !C\contains_key(keyset[TokenType::IDENTIFIER, TokenType::STRING_CONSTANT], $next['type']))
