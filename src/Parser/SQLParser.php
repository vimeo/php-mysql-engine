...
final class SQLParser {
	...
	private static function buildTokenListFromLexemes(vec<string> $tokens): token_list {
		...
		foreach ($tokens as $i => $token) {
			...
			if (Str\to_int($token) is nonnull) {
				...
				$out[$k] = $previous;
				if (
					!C\contains_key(keyset[TokenType::NUMERIC_CONSTANT, TokenType::STRING_CONSTANT, TokenType::NULL_CONSTANT, TokenType::IDENTIFIER], $previous['type']) &&
					$previous['value'] !== ')'
				) {
					$out[] = shape(
						'type' => TokenType::IDENTIFIER,
				...
				if (
					!C\contains_key(
						keyset[TokenType::NUMERIC_CONSTANT, TokenType::STRING_CONSTANT, TokenType::NULL_CONSTANT, TokenType::IDENTIFIER],
						$previous['type'],
			...
			} elseif (C\contains_key(self::OPERATORS, $token_upper) && !self::isFunctionVersionOfOperator($token_upper, $i, $count, $tokens)) {
				$out[] = shape(
					'type' => TokenType::OPERATOR,
					'value' => $token_upper,
					'raw' => $token,
	 ...
	 */
	private static function isFunctionVersionOfOperator(string $token_upper, int $i, int $count, vec<string> $tokens): bool {
	...
	*/
	public static function skipIndexHints(int $pointer, token_list $tokens): int {
		...
		while ($next !== null && $next['type'] === TokenType::RESERVED && !C\contains_key(keyset['USE', 'IGNORE', 'FORCE'], $next['value'])) {
			$pointer += 2;
